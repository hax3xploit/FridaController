...
setTimeout(function() {
	Java.perform(function() {
		...
		Java.perform(function () {
		    try {
		        var CustomTrust = Java.use('com.prosoftnet.android.idriveonline.ssl.CustomTrust');

		        // Hook the checkServerTrusted method
		        CustomTrust.checkServerTrusted.overload('[Ljava.security.cert.X509Certificate;', 'java.lang.String').implementation = function (chain, authType) {
		            try {
		                // Skip certificate validation logic and return successfully
		                console.log('[+] Bypassing certificate validation for CustomTrust');
		            } catch (error) {
		                console.error('Error while bypassing certificate validation:', error);
		            }
		        };
		    } catch (error) {
		        console.error('Error while hooking CustomTrust:', error);
		    }
		});
